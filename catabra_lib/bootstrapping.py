...
class Bootstrapping:
    ...
    ----------
    *args : array-like
        Data, non-empty sequence of array-likes with the same length.
    ...
    fn : callable | dict | tuple, optional
    ...
    def __init__(
        ...
        *args,
        ...
        self._args = args
        assert self._args
        assert all(1 <= np.ndim(a) <= 2 for a in self._args)
        self._n = len(args[0])
        assert all(len(a) == self._n for a in self._args[1:])
        self._kwargs = {} if kwargs is None else kwargs
        assert all(1 <= np.ndim(a) <= 2 for a in self._kwargs.values())
    ...
    def run(self, n_repetitions: int = 100, sample_indices: Optional[np.ndarray] = None) -> "Bootstrapping":
        ...
        if sample_indices is not None:
            ...
            n_repetitions = len(sample_indices)
    ...
    def agg(self, func):
        ...
        ----------
        func : str | callable
            The aggregation function to apply. If a string, can be the name of a Numpy function ("mean", "std", etc.),
        ...
        if isinstance(func, str):
            if func == "iqr":
                return self.iqr()
            elif func.startswith("ci") and len(func) > 2:
                try:
                    alpha = float(func[2:])
                except ValueError:
                    pass
    ...
    def interquartile_range(self):
        return self.quantile(q=[0.25, 0.75])

    iqr = interquartile_range  # alias

    def confidence_interval(self, alpha=0.95):
        return self.quantile(q=[0.5 - 0.5 * alpha, 0.5 + 0.5 * alpha])

    ci = confidence_interval  # alias

    def dataframe(self, keys=None) -> Optional["pandas.DataFrame"]:  # noqa F821 # type: ignore
        ...
        """
        if pd is None:
            raise ValueError("Method `boostrapping.dataframe()` can only be called if pandas is installed.")

        if isinstance(self._results, list):
            if all(np.ndim(r) == 0 for r in self._results):
        ...
        if keys is None:
            ...
            keys = [keys]
        res = {k: v for k, v in res.items() if k in keys and isinstance(v, list) and all(np.ndim(x) == 0 for x in v)}
        if res:
            return pd.DataFrame(res)
        else:
            return None

    def describe(self, keys=None) -> Union["pandas.Series", "pandas.DataFrame"]:  # noqa F821 # type: ignore
        ...
        """
        if pd is None:
            raise ValueError("Method `boostrapping.describe()` can only be called if pandas is installed.")
